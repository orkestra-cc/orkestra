...
- **Invoke the `frontend-design` skill before any `frontend-admin/` UI work** — including full-stack features whose UI lands in the operator console. Invoke it while planning the feature, not when you reach the JSX; it enforces the reference-first workflow (`src/reference/*.tsx` + production-page precedent)
