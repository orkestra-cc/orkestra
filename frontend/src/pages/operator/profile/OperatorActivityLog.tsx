...
import { Card, Badge, Button, ButtonGroup, Alert, Spinner } from 'react-bootstrap';
...
const OperatorActivityLog: React.FC<OperatorActivityLogProps> = ({ userId, ...rest }) => {
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case 'permission': return 'user-cog';
        ...
        {isLoading ? (
          ...
          activities.map((activity, index) => (
            ...
            >
              <Flex>
                <div className="me-3">
                  <div className={`bg-${getStatusVariant(activity.status)} rounded-circle p-2 d-flex align-items-center justify-content-center`} style={{ width: '32px', height: '32px' }}>
                    <FontAwesomeIcon
