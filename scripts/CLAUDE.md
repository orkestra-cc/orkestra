...
- **scripts/backup-cron.sh** — non-interactive nightly wrapper around `backup.sh --yes all`, meant for a `cron.d` stanza (`/etc/cron.d/orkestra-<stack>-backup`). It does not trust `backup.sh`'s exit code: it re-opens the produced tarball, checks the manifest lists all four components and that the Mongo archive has real bytes, so a future regression cannot turn the schedule back into a silent no-op. `chmod 600`s the tarball, prunes past `BACKUP_RETENTION_DAYS` (default 30) but always keeps at least `BACKUP_MIN_KEEP` (default 3), and logs every run to `backups/backup-cron.log`. Nothing in this repo installs it — see [docs/site/operating/backup-and-restore.mdx](../docs/site/operating/backup-and-restore.mdx#scheduled-backups).
...
- **devtoken.sh**: Generates dev JWT tokens for testing (`ORKESTRA_API_URL=... ./scripts/devtoken.sh administrator`). Pass `--audience client` (or `-a client`) to mint an `aud=client` token that satisfies the `api.*` surface; default is `operator` (ADR-0003 PR-D D-10). **Local development only** — the backing `POST /dev/token` endpoint is unauthenticated and mints a `super_admin` token to any caller, so it is gated on `IsProductionLike()` and does not exist in production *or* staging. Token lifetime follows the deployment's access-token TTL and is not settable per request (there is no `--expiry` flag).
...
- **health-check.sh**: Post-deploy verification, called by `orkestra.sh deploy` (gating) and `orkestra.sh status` (informational). Usage: `scripts/health-check.sh <env> [scope]` where scope is `all` (default) | `backend` | `frontend-admin` | `frontend-admin+backend` | `infra`. Exit `0` = every in-scope service healthy, `1` = at least one in-scope failure, `2` = bad usage. Checks container state plus an HTTP `/health` probe on `127.0.0.1:<port>` for `backend`/`frontend-admin`/`client-frontend`, and Docker health only for `mongodb`/`redis`/`rustfs`. **Services outside the deploy scope are still probed but only warn**, so a `--scope backend` deploy is not failed by an unrelated pre-existing frontend problem. `client-frontend` is skipped when the stack does not define it. Resolves container names as `${APP_NAME}-<svc>-${ENV}` from `docker/.env` — never guessed, because one machine runs several stacks from different checkouts. Until 2026-08-07 this file did not exist and the seam in `orkestra.sh` silently skipped on staging and hard-failed every production deploy.
