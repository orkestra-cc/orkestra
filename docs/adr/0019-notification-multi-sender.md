...
`EmailMessage`, the struct `Send` receives, grows to carry the routing category, the type and the per-message UUID alongside the rendered content. All three already exist at the chokepoint, and the struct is core-private — not `iface`, not a wire type — so this is data-carrying with no fork surface. Without the category a driver cannot express what it is sending, which is what MailUp's `CampaignCode` needs; without the UUID the deferred delivery-feedback work would have to reopen this seam to correlate a webhook back to a message.
