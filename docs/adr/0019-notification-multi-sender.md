...
`EmailDriver` (`Name` / `Requires()` / `Send(ctx, profile, msg)`) with a registry. `Requires()` declares the sub-fields the driver cannot send without, each flagged secret or not; a package-level `ValidateProfile(driver, profile, view)` reads that one declaration for both the send path (every requirement) and the save-time gate (non-secret ones only), so the two checks cannot drift. The contract deliberately expresses only "field non-empty": a driver that one day needs a richer check adds a method then, under a recorded decision, rather than carrying an empty `Validate` now. The existing SMTP transport retires *inside* the `smtp` driver unchanged; only the source of its credentials moves. A `mailup` driver for MailUp's SMTP+ REST API ships in the base.
...
"Per-environment" is only true if a send never mixes environments: the loader reads the module document once per send and decodes values **and** secrets from that one snapshot's active environment, replacing today's nine separate reads, between any two of which an activation could land.
...
So `email.provider`, `email.smtp.*` and `email.from_*` stay. **Until some profile declares a pattern** — the roster is empty, or holds only drafts — the resolver synthesizes a legacy profile from them with `categories = *` (slug `_legacy`, deliberately outside the record-list slug grammar so no roster element can collide with it). The cutover is the existence of a routing map, the same predicate D5's validator uses: saving a first pattern-less profile, or removing the last pattern, never changes which sender carries mail, and a draft stays reachable by slug for the test send. An installation that never opens the new screen behaves **identically to today**; a stack configured by `SMTP_HOST` keeps working. No data migration, no boot-time write, no rollback path to invent.
...
`IsConfiguredFor` resolves the category — with the same `TenantID` the dispatch path passes — then reports whether its profile's driver is registered and `ValidateProfile` passes with secrets in view. A category that matches no profile returns **false** — consistent with D5, and strictly better than today for that case: the signup is refused up front rather than accepted against mail that cannot be sent.
...
- **Driver errors are bounded and sanitized before they are stored — structurally.** `NotificationDoc.Error` is served by the admin delivery log and rides the GDPR export path, so no driver's raw vendor response is ever persisted: a driver returns a typed `SendError` with no free-text field, callers receive only the same bounded reason (the driver's own error is dropped, never wrapped — `auth` logs what it receives), and the chokepoint records the HTTP status and the vendor's own status and code, each through a character allowlist and a length cap, and **no vendor free text at all**. The vendor's error message is dropped rather than truncated, because MailUp carries its SMTP+ secret in the request body's `User` field: an API that echoed the request back would put that credential inside the message, where a length cap passes it through unchanged. Redacting it was rejected as a blocklist that fails silently against any encoding we did not anticipate. The response is also **read** under a bound rather than merely stored under one — 64 KiB through an `io.LimitReader`, before any parse, with an explicit client timeout beside it — since a rule about what is persisted does nothing about what an uncontrolled host can make the process allocate first. The rule is **no string produced by a remote peer is persisted at all**, which closes a pre-existing hole as well as the new one: `smtp` stores `sendErr.Error()` today, and a server is free to echo the `PlainAuth` argument — `base64(\0user\0pass)` — into its rejection line. A server rejection therefore keeps only the numeric SMTP code, a local failure only a kind from a fixed set, and both carry the name of the step that failed, which is ours and is the diagnostic that was actually useful.
...
- **Category-aware pre-flight stops silently lying, and catches what the save gate cannot.** `IsConfigured(ctx)` itself is unchanged and stays deliberately coarse — it still answers only for the default profile. What changes is that a caller about to send now asks about *that* category (D7), and because the check runs inside the module it can see secrets, so a profile missing only its API key is caught before the mail is attempted rather than after.
