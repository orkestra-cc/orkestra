...
- **Phase F** ✅ shipped 2026-05-16: `/admin/observability/log-levels` UI for runtime log-level mutation. New `logging` core module owns the `log_levels` Mongo collection (single-document, `_id=default`); `LogLevelService` holds the threshold map behind an `atomic.Pointer` snapshot replaced wholesale on every admin write. `PerModuleLevelHandler` (Phase C) refactored to use a `LevelResolver` interface backed by a shared `resolverBox` so clones produced by `deps.Logger.With("module", X)` observe live mutations through the same atomic. `main.go` hot-swaps the boot env-driven resolver to the DB-backed live resolver after `InitAll`. Frontend page lives under `frontend-admin/src/pages/admin/observability/log-levels/`. Loki retention overrides remain a future amendment.
