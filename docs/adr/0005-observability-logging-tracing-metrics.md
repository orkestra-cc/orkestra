...
The `logging` core module exposes the operator-only runtime API under `/v1/admin/observability/log-levels`. `PUT /v1/admin/observability/log-levels` atomically replaces the complete permanent snapshot with optimistic concurrency; the single-global and single-module endpoints remain focused convenience operations. Every mutation persists before publishing a new `atomic.Pointer` snapshot, so log calls see either the old configuration or the complete new configuration, never a partial map. Temporary diagnostics can start or replace a module override for 15, 60, or 240 minutes, or with no expiry; expiration is enforced on the resolver hot path and a best-effort lifecycle cleanup later removes stale persisted records.

The specialized workspace is `/admin/modules/logging`, with URL-synced `overview`, `levels`, `diagnostics`, and `logs` sections. `/admin/observability/log-levels` remains a history-replacing legacy redirect to `/admin/modules/logging?section=levels`; there is no duplicate standalone “Log levels” navigation item. The workspace's Loki preview accepts only closed filters and returns at most 100 minimized recent events. It is a bounded diagnostic aid, not an unbounded query console or stream; Grafana remains responsible for full search, streaming, and investigation.
...
- **Phase F** ✅ shipped 2026-05-16: runtime logging operations. The `logging` core module owns the single-document `log_levels` collection (`_id=default`) and publishes an atomic, persist-before-publish permanent snapshot through `LogLevelService`; its diagnostic lifecycle supplies expiring per-module overrides plus best-effort cleanup. `PerModuleLevelHandler` (Phase C) uses a `LevelResolver` behind a shared `resolverBox`, so `deps.Logger.With("module", X)` clones observe live mutations. `main.go` swaps the boot env resolver for the DB-backed resolver after `InitAll`. The authoritative frontend is the specialized `/admin/modules/logging` workspace; the old `/admin/observability/log-levels` location history-replaces to its `?section=levels` view. It also provides a constrained, 100-event Loki preview while Grafana remains the full-query and streaming console. Loki retention overrides remain a future amendment.
