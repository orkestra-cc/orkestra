...
A small extension to the slog handler reads `LOG_LEVEL_<MODULE>` env vars (e.g. `LOG_LEVEL_RAG=debug`) and gates by the `module` attribute. Default behavior (no per-module env var) uses the global `LOG_LEVEL`. Those variables seed the boot snapshot; once an operator writes runtime configuration, the persisted `log_levels` document is authoritative until reset.
...
The `logging` core module exposes the operator-only runtime API under `/v1/admin/observability/log-levels`. `PUT /v1/admin/observability/log-levels` atomically replaces the complete permanent snapshot with optimistic concurrency; the single-global and single-module endpoints remain focused convenience operations. MongoDB is authoritative across replicas: every stored mutation compares monotonic document `revision`, permanent writes alone advance `permanentRevision`, and a two-second refresh bounds replica staleness. Every mutation persists before publishing a new `atomic.Pointer` snapshot, so log calls see either the old configuration or the complete new configuration, never a partial map. Temporary diagnostics can start or replace a module override for 15, 60, or 240 minutes, or with no expiry; expiration is enforced on the resolver hot path and a best-effort lifecycle cleanup later removes stale persisted records without advancing the permanent editor token.

The specialized workspace is `/admin/modules/logging`, with URL-synced `overview`, `levels`, `diagnostics`, and `logs` sections. `/admin/observability/log-levels` remains a history-replacing legacy redirect to `/admin/modules/logging?section=levels`; there is no duplicate standalone “Log levels” navigation item. The workspace's Loki preview accepts only closed filters in a POST body and returns at most 100 minimized recent events with `Cache-Control: private, no-store`; changing draft filters does not query until the operator applies them. It is a bounded diagnostic aid, not an unbounded query console or stream; Grafana remains responsible for full search, streaming, and investigation.
...
- **Phase F** ✅ shipped 2026-05-16: runtime logging operations. The `logging` core module owns the single-document `log_levels` collection (`_id=default`) and publishes an atomic, persist-before-publish snapshot through `LogLevelService`; Mongo CAS revisions plus bounded refresh make it safe across replicas, while the distinct permanent revision keeps diagnostic maintenance from conflicting with durable edits. `PerModuleLevelHandler` (Phase C) uses a `LevelResolver` behind a shared `resolverBox`, so `deps.Logger.With("module", X)` clones observe live mutations. `main.go` swaps the boot env resolver for the DB-backed resolver after `InitAll`. The authoritative frontend is the specialized `/admin/modules/logging` workspace; the old `/admin/observability/log-levels` location history-replaces to its `?section=levels` view. It also provides a constrained, no-store, 100-event Loki preview while Grafana remains the full-query and streaming console. Loki retention overrides remain a future amendment.

Each phase is independently shippable. A and B were the minimum to claim the selling-feature framing; C–F closed the operator-experience loop: per-module debug exists at the env layer (C), as a vendor signal (D, E), and as a one-click admin toggle (F).
...
1. ~~**Per-module level mutation at runtime**~~ — Resolved 2026-05-16. Phase F shipped with a dedicated `log_levels` Mongo collection (one document, `_id=default`), not `module_configs`. Log level is treated as an observability property, not a module property — the storage shape mirrors the conceptual split.
