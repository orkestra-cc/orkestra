...
- Item 14 Compliance SOC2 deep card content (~120 evidence rows), ~~Identity `IdPConfigForm` + `ScimTokenSection`~~ ✅ extracted (`auth.identity.idpConfig.*` ~35 keys + `auth.identity.scimToken.*` ~25 keys; covers OIDC form labels/help/claim fields/delete flow and SCIM rotate/generate/reveal/status text — these components had been EN-only so IT side is a fresh translation, not a JSX-preserve), Marketing import wizard steps, Sales settings prompt template editor, AI/Graph addon page bodies
