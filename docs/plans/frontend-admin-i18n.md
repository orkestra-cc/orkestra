...
- Item 14 Compliance SOC2 deep card content (~120 evidence rows), ~~Identity `IdPConfigForm` + `ScimTokenSection`~~ ✅ extracted (`auth.identity.idpConfig.*` ~35 keys + `auth.identity.scimToken.*` ~25 keys; covers OIDC form labels/help/claim fields/delete flow and SCIM rotate/generate/reveal/status text — these components had been EN-only so IT side is a fresh translation, not a JSX-preserve), Marketing import wizard steps, ~~Sales settings prompt template editor~~ ✅ extracted (`sales.settings.*` ~80 keys spanning the 734-line page: tab nav, PromptTable + PromptEditor with `{{chars}}/{{lines}}` stats + `{{count}} chars` pluralized + reset-default confirm dialog, LLMConfigTab (model/temperature/maxTokens/locale/batch with all help text), Active Configuration sidebar with templated `{{name}} ({{modelName}}) — system default` strings), AI/Graph addon page bodies
