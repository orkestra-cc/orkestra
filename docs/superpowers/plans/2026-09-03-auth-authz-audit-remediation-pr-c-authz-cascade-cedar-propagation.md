...
**Spec:** `docs/superpowers/specs/2026-09-03-auth-authz-audit-remediation-design.md` **v1.12** — this plan implements the **PR C** row of §7, i.e. §4.4 (**D21, D22**), §4.5 (**D23, D24**; **D25 is explicitly out of scope**) and §4.6 (**D26–D29**), plus the §4.11 documentation lines and the §6 "PR C — authz" test list.
...
- **A refused pre-invalidation is a refused change.** 503, nothing written. A change whose effect cannot be guaranteed is not applied. An invalidator absent from the registry is treated the same way **for mutations** — the "degrade, do not fail" note on the interface applies to consumers that only *read* verdicts.
...
- **`DeleteRole` is unchanged.** It is already scoped and cascade-free by design: deleting a role removes access.
...
- **Docs move in the same commit as the code:** `backend/internal/core/authz/CLAUDE.md`, `backend/internal/core/user/CLAUDE.md`, `backend/pkg/sdk/CLAUDE.md`, `docs/site/modules/core/authz.mdx`, `docs/site/modules/core/user.mdx`.
