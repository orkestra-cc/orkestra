...
2. Fills six `REPLACE_WITH_RANDOM_HEX_*` placeholders with `openssl rand` output: `COOKIE_SECRET`, `OAUTH_TOKEN_ENCRYPTION_KEY`, `ORKESTRA_KMS_MASTER_KEY`, `MONGO_ROOT_PASSWORD`, `REDIS_PASSWORD`, `STORAGE_SECRET_KEY` (the last one doubles as the bundled RustFS root secret — no compose file carries a fallback for it).
...
- **Take your fork private, or run several products** → see [Private forks & the commons chain](/getting-started/private-forks)
