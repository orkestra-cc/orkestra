...
```mermaid
flowchart TB
  subgraph pub["PUBLIC — visible to everyone"]
    Upstream["orkestra-cc/orkestra<br/>upstream, core-only"]
  end
  subgraph priv["PRIVATE — your org"]
    Commons["your-org/orkestra-commons<br/>core + your reusable addons<br/>★ day-to-day work happens here"]
    A["product fork A<br/>+ addons only A needs"]
    B["product fork B<br/>+ addons only B needs"]
    C["product fork C<br/>…"]
  end
  Upstream -->|"merge upstream/dev<br/>once per release"| Commons
  Commons -.->|"cherry-pick, only Prop: upstream commits, via PR<br/>never a wholesale merge"| Upstream
  Commons -->|"merge commons/dev → dev<br/>selective: you pick which forks, when"| A
  Commons --> B
  Commons --> C
```
...
```mermaid
flowchart LR
  Clone["any private clone<br/>commons or a product fork"]
  Public["orkestra-cc/orkestra<br/>public upstream"]
  Public -->|"git fetch upstream ✓<br/>fetch URL: the real repo"| Clone
  Clone --x|"git push upstream ✗<br/>push URL = DISABLED"| Public
```
...
Re-verify this before every upstream cherry-pick round, not just once. The guard lives in each clone's `.git/config` — it isn't versioned, doesn't travel with merges, and no CI checks it, so a fresh clone, a new worktree, or a remote-rewiring script resets it silently.
