...
Open the console on `localhost`, not on `console.localhost`. Its SPA calls whatever `VITE_API_URL` says — `http://localhost:3000` by default — with credentialed requests, and a refresh cookie is `SameSite=Lax`, so **the console's own origin and `VITE_API_URL` must be the same site**: both `localhost` (the shipped pairing above), or both `console.localhost` (which also needs `VITE_API_URL=http://console.localhost:3000` and `console.localhost` in `VITE_ADMIN_ALLOWED_HOSTS`, since the console dev server does not allow that hostname by default). One of each silently breaks session refresh — see [Refresh + cookie scope](/architecture/authentication-flow#6-refresh--cookie-scope). `./orkestra.sh deploy` refuses to start a stack whose `FRONTEND_URL` and `VITE_API_URL` name different **sites** — under `*.localhost`, different hosts.
