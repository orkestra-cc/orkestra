...
`frontend-admin` serialises the rotation across tabs with a Web Lock and coalesces concurrent 401s onto one in-flight refresh, tolerating one `409 refresh_rotation_raced` retry (a sibling rotated first inside the 10-second `RefreshRotationGrace`, the family is intact, and the jar already holds the successor). It also rotates **proactively**: any non-auth request whose token expires within 30 s first awaits the same serialised refresh, so expiry rarely lands on a burst of parallel requests (the skew is deliberately below the 60 s minimum access-token TTL, so a token at the floor is never refreshed on every request). Its reactive path carries no token-state gate: apart from an auth-endpoint allowlist and the terminal session codes, a 401 is refreshed and retried on its status alone.
