...
- Prunes tarballs older than `BACKUP_RETENTION_DAYS` (default 30) — but always keeps at least `BACKUP_MIN_KEEP` (default 3) of them regardless of age, so an infrequently-running or freshly-scheduled host can never prune its way down to zero backups in one pass.
