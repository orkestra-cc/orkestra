...
| `redis` | Synchronous `SAVE`, then `docker cp` of `dump.rdb` + the AOF directory from whatever path `CONFIG GET dir` reports (`/data` since the compose command passes `--dir /data`; older stacks wrote to `/`, outside the volume, and lost everything on recreate) | Stop container, copy files back into the live server's `dir`, start container |
...
Neither `./backup.sh` nor `scripts/backup-cron.sh` schedules itself. Nothing in this repo creates a cron entry, a systemd timer, or a compose sidecar — a fresh checkout runs no backups at all until an operator installs one. Don't improvise a bare `mongodump` cron job for this (see the [manual recipes](#manual-recipes) below for when that's actually the right tool) — `scripts/backup-cron.sh` already exists for exactly this purpose:

- Runs `./backup.sh --yes all` (every component, no prompts).
- **Verifies the result rather than trusting the exit code** — re-opens the produced tarball, confirms the manifest lists all four components, and confirms the Mongo archive has real bytes, so a future regression in `backup.sh` can't turn the schedule back into a silent no-op.
- Applies `chmod 600` to the tarball (belt-and-braces on top of `backup.sh`'s own `umask 077` + `chmod 600`).
- Prunes tarballs older than `BACKUP_RETENTION_DAYS` (default 30) — but always keeps at least `BACKUP_MIN_KEEP` (default 3) of them regardless of age, so an infrequently-running or freshly-scheduled host can never prune its way down to zero backups in one pass. Both are validated as non-negative integers before anything is deleted (exit 2 otherwise): bash resolves an unknown identifier to 0 in arithmetic, so an unvalidated `BACKUP_MIN_KEEP=on` would quietly mean *keep none* — the floor vanishing at exactly the moment someone thought they were setting it.
- Verifies the archive **this run** produced, not whichever tarball in `backups/` happens to be newest, so a concurrent or hand-started backup cannot end up being the one the schedule checks and vouches for.
- Logs every run, pass or fail, to `backups/backup-cron.log`.

Install it as a `cron.d` stanza — this is the actual stanza this project uses, adjust the run-as user and the repo path for your deployment:

```cron
# /etc/cron.d/orkestra-<clone>-backup
# Orkestra nightly backup — 03:00 UTC.
SHELL=/bin/bash
PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
0 3 * * * <deploy-user> /path/to/orkestra/scripts/backup-cron.sh
```

```bash
sudo chmod 644 /etc/cron.d/orkestra-<clone>-backup
```

Run it as the same non-root user that has Docker socket access and owns the repo checkout — not root. `cron` itself is not guaranteed to be installed or running on a fresh VM; check both before assuming the schedule will actually fire:

```bash
sudo apt-get update && sudo apt-get install -y cron
sudo systemctl enable --now cron
sudo grep -R orkestra /etc/cron.d/
systemctl is-active cron
```

The manual `mongodump` recipe directly below is the fallback for hosts where Docker isn't available at all — both `./backup.sh` and `scripts/backup-cron.sh` assume `docker` is present and do not work without it.
...
Neither bundled script moves anything off the host: `./backup.sh` writes a plain `tar.gz` under `./backups/` and `scripts/backup-scheduled.sh` prunes it to the last `ORKESTRA_BACKUP_KEEP` (7) archives. A disk failure, a bad `rm`, or ransomware on the VM takes the backups with the data.

Two properties the off-site copy must have:

- **Encrypted before it leaves the host.** The `secrets` component puts `docker/.env` (Mongo root password, cookie secret, `OAUTH_TOKEN_ENCRYPTION_KEY`, `ORKESTRA_KMS_MASTER_KEY`) and the RS256 JWT private key in the tarball in clear. Whoever can read the remote copy can impersonate every user and decrypt every stored secret. `scp` of the raw tarball to a NAS is not acceptable.
- **Not writable from the production host beyond append.** A credential that can delete the remote copies is a credential ransomware on the VM can use. Prefer a remote user restricted to the backup path, and let the *remote* side (or a separate host) run retention.

### Recommended: restic over SFTP to another host

[restic](https://restic.net/) encrypts client-side, deduplicates (nightly Mongo dumps share most of their blocks), and needs only an SSH account on the far side.

```bash
# On the production VM, once
sudo apt-get install -y restic
sudo install -m 600 -o deploy -g deploy /dev/null /etc/orkestra/restic.env
cat | sudo tee /etc/orkestra/restic.env >/dev/null <<'EOF'
RESTIC_REPOSITORY=sftp:backup@backup-host.example.internal:/srv/restic/orkestra
RESTIC_PASSWORD_FILE=/etc/orkestra/restic.password
EOF
sudo sh -c 'umask 077; openssl rand -base64 32 > /etc/orkestra/restic.password'
# Keep a copy of restic.password somewhere that is NOT this VM — without it
# the repository is unreadable, which is the point.

set -a; . /etc/orkestra/restic.env; set +a
restic init
```

Then, after each scheduled backup:

```bash
set -a; . /etc/orkestra/restic.env; set +a
restic backup --tag orkestra /opt/orkestra/backups
restic forget --keep-daily 14 --keep-weekly 8 --keep-monthly 6 --prune
```

Wire the two commands into `scripts/backup-scheduled.sh` (after the local prune) or into a second `ExecStart=` line of `orkestra-backup.service`, so the off-site push shares the timer and its failure shows up in `systemctl --failed`. The SSH key for `backup@backup-host` should be dedicated to this job and, on the receiving side, pinned with a `command=` restriction to an SFTP-only shell (`internal-sftp`) chrooted to the repository path.

Object storage instead of SFTP is the same recipe with `RESTIC_REPOSITORY=s3:...` plus the bucket credentials in `restic.env` — give the key write-but-not-delete permission (S3 object lock or a bucket policy without `s3:DeleteObject`) to keep the append-only property.

### Verify the off-site copy, not just the upload

```bash
restic snapshots --latest 3
restic check --read-data-subset=5%          # monthly: actually reads a sample
restic restore latest --target /tmp/restic-verify --include '*/orkestra-backup-*'
./restore.sh --dry-run /tmp/restic-verify/…/orkestra-backup-<env>-<stamp>.tar.gz
```

Fold this into the [restore-test cadence](#restore-test-cadence) below: a restore drill that starts from the *remote* copy is the only proof the whole chain works.
...
- **Index recreation** — `--drop` removes indexes too. The module registry's `ensureCollection` recreates the declared indexes on next boot, but background index builds take time on large collections — expect slow queries for a few minutes post-restore. Note the recreation only works because `--drop` left nothing behind: `ensureCollection` is **create-only**, so if an index survives with a specification that no longer matches its Go declaration, it is never repaired — boot logs an error and moves on. See `docs/migrations/` in this repo for the reconcile-and-recreate runbook that closes a drift like that.
...
| Date | Stack | Archive | Result |
| --- | --- | --- | --- |
| _(none recorded)_ | | | Add one row per drill: what was restored, into what, and what you compared against. |

Restoring into a renamed namespace rather than over the live database is the safer shape for a drill and is what the table above is for — it exercises the archive end-to-end without putting the running stack at risk. It does **not** exercise `--drop` semantics or a full stack boot against restored data, so it is a floor, not a substitute for the staging-stack drill described above.
