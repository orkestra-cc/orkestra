...
| `rustfs` | Enumerates buckets via `aws-cli` and `s3 sync`s each one out. The helper container joins whatever network the running `rustfs` container is on — resolved with `docker inspect`, preferring the stack's own `${STACK}_default` when it is still attached — so a stack started under a custom `COMPOSE_PROJECT_NAME` works too. A failed `s3 sync` is a hard error: the component is reported failed and left out of the manifest, instead of being archived as an empty tree | `s3 mb` (best-effort) + `s3 sync` from the bundle back into each bucket, with the same network resolution and the same hard failure on a failed sync |
