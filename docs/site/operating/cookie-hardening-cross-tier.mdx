...
**Both domains are empty on purpose**, and that is the whole dev configuration. An empty value writes no `Domain` attribute at all, so each refresh cookie is host-only — scoped to exactly the host that minted it, which round-trips on `localhost`, on `*.localhost` and on a LAN IP for multi-device testing (an IP literal cannot be a cookie `Domain`). This is the shipped default in **every** environment (`config.go:467-474`), not a dev-only relaxation.

:::note
`COOKIE_DOMAIN` is **not read by the backend** — `OPERATOR_COOKIE_DOMAIN` and `CLIENT_COOKIE_DOMAIN` are the only inputs (`config.go:321-322`). The `COOKIE_DOMAIN` lines in the deployment examples further down are inert; purging them is a pending docs follow-up.
:::
