...
`OAUTH_GOOGLE_REDIRECT_URL` and friends only *seed* that field, and they seed it on any boot at which the key is still absent — not just the first — so uncommenting one and recreating the container does take effect; once a value is stored, the stored value wins and the variable is ignored. Change it in the admin UI (or `PATCH /v1/admin/modules/auth`). (A field that resolves empty — nothing stored *and* no env var set — is a different symptom: the web flow refuses the provider before it ever reaches the IdP.)
