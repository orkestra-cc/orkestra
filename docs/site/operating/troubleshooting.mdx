...
1. **Wrong host registered.** In development the backend computes `http://localhost:3000/v1/auth/oauth/google/callback` — the compiled default, and the operator host under the shipped `localhost` convention — so a callback registered on `console.localhost:3000` (or any other host) mismatches. Re-register with the host the backend actually answers the login POST on, or set `OAUTH_GOOGLE_REDIRECT_URL` to the host you registered.
