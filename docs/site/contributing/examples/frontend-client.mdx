...
- **Auth flow**: standard external-client registration, login, and account self-service. Post-[ADR-0006](/adrs/collapse-to-core-only-base) it's a thin login + account + billing-identity skeleton — the subscribe / payment-method / transaction flows left with the addons. A fork that sells services to external clients rebuilds the consumption surface here.
...
[`orkestra-cc/orkestra/tree/main/frontend-client/`](https://github.com/orkestra-cc/orkestra/tree/main/frontend-client) — start with `src/api/` for the consumer surface and `src/routes/` for the registration + account flows.
