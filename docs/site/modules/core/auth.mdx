...
that would 403 on submit. The **operator console** does this today; the
client SPA gains the same gating with the client OAuth login work — until
then it reads only `loginEnabled` / `registrationEnabled` from the same
endpoint, so its login page still renders a form the backend refuses with
403 on submit.
