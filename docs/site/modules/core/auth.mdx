...
The bootstrap endpoint `GET /v1/auth/session` shares only the first half of
that. Its cookie-classification step answers 503 the same way, but the
read-only mint it performs afterwards (`MintAccessTokenFromRefresh`) is not
yet classified, so a store failure opening between that call's own lookup and
its mint still answers a codeless 401. Closing that gap is a tracked
follow-up; until then, do not read the paragraph above as covering the whole
refresh surface.
