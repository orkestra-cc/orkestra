...
risk scorer reads — nothing authenticates off them — and are deleted 90 days
after `expiresAt` by a Mongo TTL index. Refresh-**token** rows have their own
retention story, covered next.
