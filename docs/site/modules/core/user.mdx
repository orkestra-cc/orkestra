...
- **The system role** — `User.Role`, one of `super_admin` › `administrator` › `developer` › `manager` › `operator` › `guest`. This is the value the JWT carries as `srole`. Org-scoped roles are a different thing entirely and live in [authz](/modules/core/authz) as role bindings.
