...
│   │   ├── authentication/        # Login forms, ProtectedRoute, OAuth callback handlers. SocialLoginForm renders buttons from the live backend list (`useGetOAuthProvidersQuery` → `GET /v1/auth/operator/providers`) so toggling a provider on `/admin/modules/auth` removes it from the login page within 30s — never hardcode the provider list here. ProtectedRoute stashes the attempted URL in `location.state.from`; every login-completion path (password, MFA, OAuth-via-`sessionStorage`) returns the user there through `utils/returnTo.ts`'s `sanitizeReturnTo` (open-redirect + auth-loop guard, default `/dashboard/analytics`) — never hardcode the post-login destination.
...
A declared group that owns **no fields of its own** — a pure container like `oauth` sitting over `oauth.google` / `oauth.apple` — is a legitimate shape, and `ModuleConfigPanel` renders it as a table of contents of its children (each entry moves the rail there) rather than a bare heading. A _leaf_ group can land in the same "heading over an empty body" state for a different reason: it owns fields, but every one of them is currently hidden by an unmet `DependsOn` (a provider's credentials before either of its enable toggles is on) — `ModuleConfigPanel` renders an honest empty-state string (`adminModules.detail.rail.emptyUntilDependency`) instead of silently producing nothing, since it has no children to link to. The sticky save bar (`detail/index.tsx`'s `showSaveBar`) is gated on the active node owning fields that are **currently visible**, not merely declared — `useModuleConfigController` exposes a module-wide `visibleKeys` set for exactly this check — so neither a container panel nor an all-hidden leaf ever carries a permanently-disabled Save.
