...
A declared group that owns **no fields of its own** — a pure container like `oauth` sitting over `oauth.google` / `oauth.apple` — is a legitimate shape, and `ModuleConfigPanel` renders it as a table of contents of its children (each entry moves the rail there) rather than a bare heading. The sticky save bar is gated on the active node actually owning fields, so a container panel never carries a permanently-disabled Save.
