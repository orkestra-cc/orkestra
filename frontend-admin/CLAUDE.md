...
All slices extend `baseApi` via `injectEndpoints`. To add a new tag type, declare it in `baseApi.ts`'s `tagTypes` array. Auth uses **cookies + Bearer token** — `credentials: 'include'` is set in the base query, and the access token from the auth slice is added to the `Authorization` header while it is unexpired. The backend's `RequireAuth` is **bearer-only** (ADR-0020, #317): it never reads the refresh cookie, so `baseQueryWithRetry` owns the whole refresh lifecycle in three layers — (1) **proactive**: any non-auth request whose token expires within `PROACTIVE_REFRESH_SKEW_MS` (30 s — kept strictly below the backend's 60 s `MinAccessTokenTTL`; at or above it a floor-length token would refresh on every request) first awaits `performRefresh`; (2) **reactive**: a 401 on a non-auth endpoint runs the same `performRefresh` and retries once — but **only on proof the request never reached its handler**, because the retry re-sends it: either the 401 body carries `code: "access_token_expired"` (the backend's own account of rejecting an expired bearer _before_ dispatch), or the request went out with no live bearer at all, by the same `liveBearer()` predicate `prepareHeaders` uses to withhold one (on a **protected** route `RequireAuth` rejects that before dispatch too), evaluated against the token state captured **before** the fetch so a sibling tab's rotation or sign-out cannot rewrite the answer. Neither proof and the 401 goes back to the caller untouched — **no replay, ever**. It is not returned _unexamined_, though: when it also carries **no top-level code at all**, `baseQueryWithRetry` runs `performRefresh` **once** and then returns the original 401 unchanged. That third arm exists for the one input in this shape that is not a verdict — a JWT signing-key rotation, or a restart with new key material, after which every unexpired bearer validates as plain "invalid" and `RequireAuth` answers a codeless 401. Without it the console did nothing at all against that: no refresh, no toast, no sign-out, every request failing silently until the proactive check fired at `expiry − PROACTIVE_REFRESH_SKEW_MS` (up to `TTL − 30 s`, ≈14.5 min at the 15-minute default). Rotating collapses the window to a single request, because the _next_ request carries the fresh bearer. Its three outcomes: `ok` → dispatch the new token, return the 401; `retry` → keep the token, return the 401; a bare `{ ok: false }` → the refresh cookie itself was refused, which is the session's own death, so `clearAccessToken()` and the existing sign-out path. **That last outcome is the only way this arm signs anyone out, and only a `401` from `/refresh-cookie` produces it** (see the allowlist in layer 3) — so a verdict 401 whose rotation meets a rate limit or a 5xx keeps the session, which matters because a mistyped password must never be able to end one. **Codeless**, not "anything but `access_token_expired`" — a 401 that names itself has been explained by the server and a token minted from the same cookie cannot change the answer (`audience_mismatch` is the live example). **The four routes below no longer reach this arm at all**: since 2026-09-04 the backend names its verdict 401s — `auth.mfa_code_invalid`, `auth.webauthn_challenge_invalid`, `auth.webauthn_assertion_failed`, `auth.invalid_credentials` (`backend/internal/shared/errcode/codes.go`, whose block comment is the canonical statement of the rule) — so a wrong code or a wrong password is now a _coded_ 401 and comes back untouched. The arm stays because it exists for the signing-key rotation, not for verdicts, and every future backend 401 that IS a verdict must name itself for what follows. ⚠️ **"The cost is one serialised rotation per verdict 401, which is harmless" was measured wrong, and it is what made a codeless verdict dangerous.** `performRefresh` **serialises** rotations, it does not collapse them — each queued one is a real rotation — so a burst does not cost one. Live on staging, 26 mistyped MFA codes in 13 seconds produced 26 codeless 401s, 44 `409 refresh_rotation_raced` answers, 8 successful rotations and then 2 × 401: the family's reuse detection killed the session the operator was typing into. The family being "untouched" per rotation is true and beside the point; the races are the harm. The **replay** guard is what is not optional: `change-password`, `/me/password-confirm`, `mfa/verify` and `mfa/enroll/confirm` all answer 401 as a verdict on the _request_, none of them is in `AUTH_ENDPOINT_PATHS`, and re-sending one costs a second wrong-password attempt (double-counted toward the lockout on `/me/password-confirm`), a burnt TOTP replay guard, a consumed backup code, or one of five enrolment attempts. The second proof's reasoning holds only for routes behind `RequireAuth` — a **public** route runs its handler bearer-less by design, so "no bearer was sent" proves nothing there — which makes `AUTH_ENDPOINT_PATHS` part of the guard rather than just loop avoidance: every public route this SPA calls that can answer 401 must be listed, and `v1/auth/operator/mfa/webauthn/login/` (begin + finish, one substring entry) joined it for exactly that reason, its TOTP twin `mfa/login/verify` having been there all along. The second proof is also what keeps the console recovering against a backend that does not send the code — a missing-bearer 401 is codeless, and that is exactly the shape a failed proactive rotation produces (ADR-0020 D3's fallback). The two deliberate replays at `step_up_required` / `password_confirm_required` sit ahead of this branch and are unaffected — and so does a **third** gate answer that replays nothing: `reauthentication_required` (spec §4.2 D14, the no-factor branch of the backend's enrolment-proof gate) means "this session is too old to add a second factor", which no modal can answer — a step-up needs a factor the caller does not have, and a password reconfirm is refused for an MFA-obligated account inside its grace window and impossible for an OAuth-only one — so it `clearAccessToken()`s and calls `navigateToLogin(sanitizeReturnTo(currentPath()) ?? DEFAULT_POST_LOGIN)`, the console's one return-path convention (there is no `?next=` anywhere in this SPA). **`currentPath()` is `pathname + search`, shared by all four `navigateToLogin` call sites** — three of them passed the pathname alone, which was invisible for as long as `AuthProvider` stored `state.from` as a string and `locationToReturnTo` dropped it, and became a silent `?tab=addons` loss the moment the deep link started surviving. The hash is deliberately absent: it never reaches the server, nothing routes on it here, and `sanitizeReturnTo`'s checks are written against path + query. It is the only 401 branch with **no** `isAuthEndpoint` exclusion, and deliberately: that exclusion exists on its two siblings because the modal they open calls an auth route, so that route's own 401 would re-open the modal — this branch opens nothing and issues no request, so there is no loop to avoid. The path is sanitised because `window.location` is influenceable within the origin, and a rejected one degrades to `DEFAULT_POST_LOGIN` rather than to `undefined`, which `AuthProvider` would fill back in from `location.pathname` — the very value just rejected. The two enrolment dialogs (`MfaEnrollWizard`, `WebAuthnEnrollDialog`) check this code **ahead of** their bare `status === 401` arms, which otherwise flatten every 401 into "that code is incorrect" / "attestation failed"; they render nothing for it, because the redirect is already under way; (3) `performRefresh` itself is serialised — one in-flight promise per tab, a Web Lock across tabs (the **two-argument** `locks.request(name, callback)` form, pinned by an arity assertion in `baseApi.rotationRace.test.ts` because a switch to the 3-arg `request(name, {signal}, callback)` overload would otherwise fail silently), and one `409 refresh_rotation_raced` retry. `refreshOnce` bounds its attempt with `REFRESH_FETCH_TIMEOUT_MS` (10 s) so a `/refresh-cookie` that accepts the connection and never answers can't stall the outbound request behind it forever. The mechanism is an **`AbortController` + `setTimeout`, with `clearTimeout` in a `finally` and nowhere else**, and the timer spans the **body read** as well as the headers: `fetch` resolves on headers, so `res.json()` is raced against an abort rejection. The parse settles into a **discriminated** result first — a wrapper on success, `null` on any parse failure — which both keeps the losing promise from surfacing as an unhandled rejection and keeps an _unreadable_ body distinguishable from an empty one; after the race, `aborted || raced === null` is what decides, rather than inferring the outcome from who won. Without that second half a server that sends headers and then stalls the body holds the cross-tab Web Lock — and every other tab's rotation — for as long as it stalls. The outcome rule is an **allowlist**, ported from the client tier: a `401` from `/refresh-cookie` — and only a 401 — is the credential being refused, which is the session's own death and the single input that yields the bare `{ ok: false }`; a `409` keeps its one raced retry inside the lock; **everything else** is `{ ok: false, retry: true }` and keeps the token — 503, 429 (the endpoint sits under the router's global rate limiter, and a burst of tabs is what trips it), 408, any other 4xx or 5xx, a transport failure, the timeout or abort, and a 2xx whose body is **unreadable** (a captive portal's HTML, a proxy error page, a connection dropped mid-read) **or** carries no token. It was a denylist until batch 3 — only 503 and 409 were transient — and nothing tested the gap, so a rate-limited or 5xx rotation signed the operator out. Do not reintroduce a list of transient statuses to remember to extend: a response that describes the **server** has said nothing about the **session**. **Name the trade (G9):** against a `/refresh-cookie` that fails _persistently_ with anything but a 401 — a 404 from a misconfigured `apiUrl`, a gateway 502 — the console keeps the operator "in". Every rotation returns `retry`, the token is kept, the original 401 goes back to the caller, and there is no toast and no redirect, so the session degrades into silent 401s on every request. That is identical **in kind** to the 503 arm that has always behaved this way — the allowlist widened which statuses reach that arm, not what the arm does — it self-heals the moment one rotation succeeds, and it is not a trap the operator cannot leave: a reload re-bootstraps through `GET /v1/auth/session`, which fails the same way, leaves the SPA with no session, and `ProtectedRoute` navigates to the login page (a 5xx additionally raises the generic "Server error" toast). The alternative trade is strictly worse: signing out on a server-shaped answer ends live sessions on every blip. `__setRefreshTimeoutForTests` is the **test-only** seam those suites use to shorten the bound; nothing in production may call it, and fake timers are not an alternative (`performRefresh`'s own `setTimeout(…, 0)` is drained on a real timer in every suite's `afterEach`). The SPA rotates only at `POST /v1/auth/operator/refresh-cookie`; `GET /v1/auth/session` mints without rotating. Never add another caller of `/refresh-cookie` or `/refresh` — the legacy `utils/apiClient.ts`, `hooks/redux/useAuth.ts`, and `authSlice.refreshSession` were exactly that and have been deleted (#317 follow-up).
...
| `src/test/render.tsx`    | `renderWithProviders(ui, { preloadedState, store, routerEntries })` — wraps in a fresh non-persisted Redux store + `MemoryRouter`. Returns `{ store, ...renderResult }`. Also exports `waitForQuerySettled(store, endpointName)`, which resolves once that RTK Query endpoint has a **settled** (fulfilled or rejected) cache entry — the anchor for policy-gated UI whose tree is byte-identical before and after the query lands, where a DOM anchor would let a following absence assertion pass vacuously against the first paint. Prefer a DOM anchor whenever one exists |
