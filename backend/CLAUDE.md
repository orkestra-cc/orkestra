...
**Runtime logging operations** (ADR-0005 Phase F): the `logging` core module (`internal/core/logging/`) owns the `log_levels` Mongo collection and nine Tier-1 operations under `/v1/admin/observability/log-levels`: read, atomic permanent apply, legacy global/module set/unset/reset, temporary diagnostic start/stop, and a bounded POST log preview. Every route requires `system.modules.admin`. The operator UI lives at `/admin/modules/logging?section=overview|levels|diagnostics|logs`; the former `/admin/observability/log-levels` URL is redirect-only. The slog handler's `LevelResolver` is hot-swapped at boot to a Mongo-authoritative live resolver. Writes use document `revision` CAS, permanent drafts use a separate `permanentRevision`, and a two-second refresh bounds cross-replica staleness while keeping every log read lock-free. Env vars (`LOG_LEVEL`, `LOG_LEVEL_<MODULE>`) seed boot defaults; a persisted document takes precedence.
