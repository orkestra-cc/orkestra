...
| `authz.role.update` | no | Update custom roles; toggle a role's active state — on a **system** role the caller's own system role must also be `super_admin`/`administrator` (see [Key invariants](#key-invariants)) |
...
| PATCH | `/v1/tenants/{tenantId}/authz/roles/{roleId}` | Update role — custom: name/description/permissions/isActive; system: `isActive` only, and only for a platform administrator |
...
**403 `authz.platform_admin_required`** is the answer when `PATCH .../roles/{roleId}` tries to flip `isActive` on a **system** role from a caller who is not a platform administrator — a global catalog row's active flag is platform configuration, so the refusal reads the caller's own system role from the database rather than their permissions (see [Key invariants](#key-invariants)). The codeless 403 next to it, `ErrSystemRoleImmutable`, still covers name/description/permissions on the same row.
...
- **System roles are immutable on name/description/permissions, and `IsActive` — the one field that is left — is gated on the caller's own system role.** `UpdateRole` on an `IsSystem=true` row returns `ErrSystemRoleImmutable` (`services/service.go::ErrSystemRoleImmutable`) if the caller touches `Name`, `Description`, or `Permissions`. The UI enforces this too, but the service is the authoritative gate.
  - **Only a platform administrator may toggle `IsActive` on a system row** — `assertPlatformAdmin` refuses anyone whose own system role is outside `platformAdminRoles` (`super_admin`, `administrator`) with `ErrPlatformAdminRequired` → **403 `authz.platform_admin_required`**. Every seeded row is `IsSystem=true` at `tenantId=""`, the `org_*` ones included, so this covers all eleven. Without it the `{tenantId}` in the path was decorative for this one field: the tenant-scope guard skips system rows by construction (they belong to no tenant) and the immutability guard does not cover `IsActive`, so any caller holding `authz.role.update` — **`org_owner` and `org_admin` both do** — could disable `administrator` for the entire platform from inside their own tenant, and `GetEffectivePermissions` skips inactive roles, so it would grant nobody anything.
  - **The rule is about who the caller IS, not what they hold, and a cascade check is not a substitute.** An `org_owner` holds *exactly* the permissions the `org_owner` row carries, so "the actor must hold everything the role carries" passes and would still let them disable `org_owner` platform-wide (`TestUpdateRole_SystemRoleDisableRefusedThoughCascadeWouldPass` pins that the cascade passes there).
  - **`developer` is deliberately outside the allowed set.** Production seeds it read-only (D9), so admitting it would hand a production developer a platform-wide write the environment gate exists to deny — and the gate must not vary by environment. The allowed pair is the one the rest of the platform already treats as its administrator quorum (`user/services.CountActiveAdministrators`, `user/handlers.isPrivilegedSystemRole`), so "who is an administrator" has one answer across modules. Do not confuse `platformAdminRoles` with `platformSystemRoleNames`: the latter is all six platform role *names* (`guest` included) and answers a different question — which roles need a global binding.
  - **The actor's system role is read from the DATABASE** through the `UserSystemRoleLookup` (`s.userRoles`), never from the `srole` JWT claim: that claim can be a whole access-token lifetime stale, which is precisely the window the role-change propagation (D28) closes. The module's lookup keeps its dev-token fallback, so a non-production `dev-` principal still resolves and the documented local/staging flow keeps working. Two waivers and a fail-closed default: the `"system"` sentinel passes (in-process platform writes — the handler maps a request that spells it to the empty actor); an empty actor is `ErrGranterRequired` (400), which this path has to say itself because an `IsActive`-only patch never reaches the D21 validator that normally raises it; anything the lookup cannot answer — unwired, erroring, or a role outside the set — is refused, because a role that cannot be read is not a proven administrator.
  - **Ordering is pinned**: the immutability check runs first, so a patch mixing `Name` with `IsActive` is refused as immutable rather than as a gate failure.
