...
| `services/visibility.go` | Shared per-item self-gate predicate (`evalSelfVisible`, reason-coded) + gate reader (`resolveGates`) called by both the public filter and the admin matrix, plus the reason-code constants |
| `services/admin_navigation_service.go` | Unfiltered tree builder with MinRole/Tier/ModuleEnabled metadata; computes the per-(role × tenant-kind) `Visibility` truth table (+ `RequiresConfig`/`ConfigSatisfied`) and stamps `EffectiveOrder` + `Overridden` from the override layer |
...
- **The per-item self-gates are shared, not duplicated.** `services/visibility.go::evalSelfVisible` is the single predicate for the module-enabled / RequiresConfig / Tier / MinRole gates (reason-coded), and `resolveGates` reads the two role/tenant-independent gates (module-enabled, config) off the enabled-checker. Both `dynamic_navigation.go::convert` (public sidebar) and `admin_navigation_service.go::toAdminItem` (admin tree) call them, so the admin **role matrix can never drift** from what the live sidebar renders.
- **The admin tree carries a visibility truth table.** `AdminNavItem` exposes `RequiresConfig` + `ConfigSatisfied` (live gate value) and `Visibility map[role]→{internal,external}` of `{visible, reason}` cells — computed via `evalSelfVisible` for every (system role × tenant kind), with parent-collapse layered on (`reason="parent_collapsed"` when a no-path group has no visible child for that cell). Reason codes: `module_disabled | config_off | tier_mismatch | role_below_min | parent_collapsed`. The frontend `/admin/modules/navigation` renders this as the tri-state role matrix, a tenant-kind toggle, the detail-panel role×tenant grid, and the "View as" sidebar preview — it never recomputes visibility client-side.
- **Four filters run per request**, in order, in `services/dynamic_navigation.go::convert`:
  ...
  2. **RequiresConfig.** If `NavItemSpec.RequiresConfig` is set, the item is hidden unless that boolean config key of its **own** module (`ModuleName`) is truthy (`true`/`1`/`yes`). Evaluated per request by type-asserting the enabled-checker to a `configValueReader` (`GetValue`), so a `/admin/modules` config toggle reflects on the next nav fetch without a restart. This is how a sub-feature gates its nav link (e.g. compliance's `SOC2 Evidence` on `soc2_enabled`) — `NavItems()` is collected before any module's `Init`, so it cannot read Init-set state itself; emit the item unconditionally and set `RequiresConfig` instead.
  3. **Tier vs tenant kind.** Items with `Tier="internal"` are hidden from callers acting in an external tenant; `Tier="external"` items are hidden from internal callers; empty `Tier` is visible to both. Tenant kind is read from `middleware.TenantKindFromContext(ctx)` (populated by the auth middleware from the JWT's resolved acting-tenant).
  4. **MinRole.** Items are included when the caller's system role is at or above `MinRole` in the hierarchy `super_admin > administrator > developer > manager > operator > guest`. Empty `MinRole` = no restriction. The hierarchy matches `ROLE_HIERARCHY` on the frontend.
