...
- **`RoleMiddleware.RequireEnrolmentProof(maxAge)` — the enrolment proof gate** (audit findings H-2 / H-3, spec §4.2 D11/D12). Applied to the **four** endpoints that create or replace a second factor: TOTP `enroll/begin` + `enroll/confirm` and passkey `register/begin` + `register/finish`, on **both** tiers. Before it, those sat under `RequireGlobal()` alone, so a stolen session-only bearer could attach a passkey to the victim's account — or **replace** their TOTP secret, since `EnrollConfirm` deletes the existing factor after validating a code for the *new* one — and then own the account outright.
  ...
  **Fail closed.** An unwired (`nil`) or erroring `MFAEnrollmentLookup` answers `step_up_required` for everyone, without consulting freshness — a degraded Mongo must never be the reason a factor can be added without proof. A caller who holds a factor can still satisfy that; a caller who holds none cannot enrol until the lookup recovers. The sidecar `JWTValidator` mirrors that branch **unconditionally** (it has no access to the `*_mfa_factors` collections, so it can never establish presence), rather than the pass-through it uses for `RequireLowRisk`.
