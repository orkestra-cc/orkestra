...
| `JWT_ACCESS_TOKEN_EXPIRY` | Access-token TTL (e.g. `15m`, `1h`, `1d`). Applied by `NewJWTService`; zero/unset falls back to `15m`. A positive value above `MaxAccessTokenTTL` (24h) is clamped down with a warning — a token allowed to outlive the Redis revocation-denylist window would be accepted again after its own revocation entry expired. | `15m` |
