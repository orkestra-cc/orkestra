...
| `services/password_auth_service.go` | Password register/login/verify/reset/change; `Login`'s lockout runs on the `AttemptCounter` scopes (see "Attempt counters" below), the remaining flows still on the shared `RateLimiter` |
...
8. **Password auth service**: register/login/verify/reset/change flows, wired to the optional notification sender, the Redis `AttemptCounter` (`Login`'s lockout) and the shared `RateLimiter` (the flows not yet migrated off it).
...
- **Rate limiting** lives in `shared/errors.RateLimiter`, shared as ONE instance across `PasswordAuthConfig` and `NewServiceAccountService` — `module.go` constructs a single `rateLimiter` and wires it into both. Two writers remain on it: `ConfirmPasswordWithSecurity` (`recordFailed`, keys `"ip:"`/`"email:"`) and `ServiceAccountService`'s own `recordFailed` (keys `"ip:"`/`"client:"`). Because the instance is shared, `ConfirmPasswordWithSecurity`'s `"ip:"` write **is read**: `ServiceAccountService.Grant`'s `IsLockedOut("ip:"+IP)` peeks the identical bucket, so a failed password-reconfirm attempt from an address counts toward that address's client-credentials-grant lockout. Its `"email:"` write is **not** read by anything in production — `ServiceAccountService` reads only `"ip:"`/`"client:"` keys via `IsLockedOut`, and nothing in this module still calls `IsBlocked` — so that half is a genuine dead write until `ConfirmPasswordWithSecurity` migrates with its own task. `Login` no longer uses the limiter at all — its lockout is the `AttemptCounter` (see "Attempt counters" under Runtime configuration); `ForgotPassword` and `ResendVerification` moved to their own `AttemptCounter` request-cap scopes (`reset-*`/`verify-*`) in that same section. The limiter's own defaults are hardcoded; tune them in `password_auth_service.go`, not in the handler.
...
- **Never bypass the lockout on login / forgot-password / resend-verification endpoints.** `Login` must peek the attempt counters before the user lookup and record on every real failure; `ForgotPassword` and `ResendVerification` must peek `overRequestCap` and charge `chargeRequestCap` BEFORE the user lookup — together these are the only protection against credential stuffing and reset-flood. `ConfirmPasswordWithSecurity` must keep calling `recordFailed` (`shared/errors.RateLimiter`) too: its `"ip:"` write feeds `ServiceAccountService.Grant`'s own lockout check via the shared limiter instance (see the Rate limiting bullet above), even though its `"email:"` write is currently unread by anything.
