...
- **`MFAHandler.Verify` and `WebAuthnHandler.VerifyFinish` are the sixth
  ...
  **Only a rejected credential is charged** — `ErrMFAInvalidCode`, plus
  `ErrWebAuthnAssertion` on the passkey route. "Not enrolled", a purpose
  mismatch, a disabled method and a wrapped store error are refusals, not
  guesses, and charging them would let a degraded backend spend a
  legitimate user's budget without an attacker trying a single code. The
