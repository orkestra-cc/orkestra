...
| `services/password_auth_service.go` | Password register/login/verify/reset/change; `Login`, `ChangePassword` and `ConfirmPasswordWithSecurity` all lock through the `AttemptCounter` scopes (see "Attempt counters" below) — this module no longer writes to the shared `RateLimiter` |
...
8. **Password auth service**: register/login/verify/reset/change flows, wired to the optional notification sender and the Redis `AttemptCounter` — `Login`, `ChangePassword` and `ConfirmPasswordWithSecurity` all lock through it now. `PasswordAuthConfig` still carries a `RateLimiter` field (the same shared instance `module.go` also wires into `NewServiceAccountService`), but nothing in this service reads or writes it any more.
...
- **Rate limiting** lives in `shared/errors.RateLimiter`, shared as ONE instance across `PasswordAuthConfig` and `NewServiceAccountService` — `module.go` constructs a single `rateLimiter` and wires it into both. `PasswordAuthService` still carries the `RateLimiter` handle (wiring symmetry with `ServiceAccountService`'s construction), but it is now dead weight: `ConfirmPasswordWithSecurity`'s `recordFailed` call (keys `"ip:"`/`"email:"`) is gone — it migrated onto the `AttemptCounter` scopes with `Login` and `ChangePassword` — and the method itself was deleted, so this module no longer reads or writes the shared instance at all. **`ServiceAccountService`'s own `recordFailed` (keys `"ip:"`/`"client:"`) is now the ONLY writer left on it**, read back by `Grant`'s `IsLockedOut("ip:"+IP)` / `IsLockedOut("client:"+ClientID)` — a separate concern (Task 10 territory), untouched by this change. `Login`, `ChangePassword` and `ConfirmPasswordWithSecurity` all lock through the `AttemptCounter` instead (see "Attempt counters" under Runtime configuration); `ForgotPassword` and `ResendVerification` use their own `AttemptCounter` request-cap scopes (`reset-*`/`verify-*`) in that same section. The limiter's own defaults are hardcoded in `internal/shared/errors/rate_limiter.go`'s `setDefaultConfigs` (`:58-68`); `service_account_service.go` — its only remaining consumer — doesn't touch those, it *overrides* the auth-failure config per call via `SetAuthFailedConfig(s.policy.LockoutThreshold(ctx), s.policy.LockoutDuration(ctx))`, i.e. an operator tunes it at `/admin/modules`, not by editing either Go file.
