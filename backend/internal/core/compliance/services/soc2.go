...
import (
	...
	complianceModels "github.com/orkestra/backend/internal/core/compliance/models"
...
func (s *SOC2EvidenceService) privilegedUsers(ctx context.Context) (map[string]any, error) {
	...
	defer cancel()
	users := s.db.Collection("operator_users")
...
func (s *SOC2EvidenceService) mfaCoverage(ctx context.Context) (map[string]any, error) {
	...
	users := s.db.Collection("operator_users")
	// Cross-module collection-naming contract: privileged accounts are
	// operator-tier, so MFA coverage reads operator_mfa_factors. Inlined
	// (not imported from auth/models) to keep compliance import-decoupled.
	factors := s.db.Collection("operator_mfa_factors")
