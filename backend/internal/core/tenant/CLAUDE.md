...
| `repository/repository.go` | MongoDB CRUD for orgs, memberships, invites; personal-tenant predicate lookup; `CountProvisioningSlotsByKind` (single-mode invariant); `RestoreTenant` (reverses `SoftDeleteTenant`, used only by the setup-tenant reconciliation seam — see [Creation vs. reconciliation](#creation-vs-reconciliation)) |
...
- **Restoring a soft-deleted reserved row requires the caller's coordinator attestation, and is the one reconciliation branch that DOES re-apply the gate** — the latter against OTHER occupants only. A prior attempt's partial-failure rollback (`createTenantWithUUID`'s own `SoftDeleteTenant` calls on a KMS/membership/bind-owner failure) can leave the reserved row soft-deleted; restoring it re-occupies a slot, which is a state transition serious enough to require the same cardinality check a genuine creation gets. Because the row's own `deletedAt != nil` already excludes it from `CountProvisioningSlotsByKind`'s count, the check can only ever see occupants other than itself.
  **The attestation is `EnsureSetupTenant`'s trailing `coordinatorAttested bool`**: the caller's statement that the setup coordinator record (`systeminit`, `key: setup_finalization`) for *this* reservation exists and is not completed. It exists because the row signature alone cannot say *who* soft-deleted the row — `DeleteTenant`, the MFA-gated platform-admin destructive route, ends in the very same `SoftDeleteTenant` call the rollback makes. The design's rule is that restoration applies when "the **coordinator** and immutable identity prove that a prior attempt for this same setup reservation soft-deleted the row"; identity alone is only half of it. Only the setup finalization saga holds the coordinator, so only it can attest (it derives the flag from the record it just CAS-claimed a stage on — `rec.RequestHash != "" && rec.CompletedAt == nil`); every other caller passes `false` and a soft-deleted reserved row then returns `ErrSetupTenantRemediation` instead of being silently restored. The tenant service deliberately does not resolve the coordinator itself — `systeminit` is not reachable from here, and a second source of truth for it would be worse than a documented caller contract.
...
- `DeleteTenant` / `PurgeTenant` additionally run a **cheap pre-check first** — compare `DefaultTenantUUID(ctx)` against the target and bail out before `cascadeTenantData` runs, so the expensive membership/ancestor cascade never runs in the common denied case. This pre-check alone is racy (a concurrent transfer can move the default between the read and the guarded write below it), so their final row write (`SoftDeleteTenant` / `UpdateTenantStatus(purged)`) is *also* wrapped in `RunDefaultGuarded` — that guarded write, not the pre-check, is the actual invariant protecting the tenant row. A denial after the pre-check has already passed can still leave the cascade's side effects (dropped memberships/ancestors) applied even though the row itself is left unchanged; this is an accepted, documented trade-off, not a bug.
