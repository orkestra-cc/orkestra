...
| `repository/provisioning_locks.go` | `CollProvisioningLocks` + `RunProvisioningGuarded` + `ErrProvisioningSlotTaken` — the transactional cardinality guard behind `single` mode. See [Provisioning policy](#provisioning-policy-admin-managed) |
...
| `tenant_provisioning_locks` | `kind` unique | — — **platform-global, no `tenantId`**: one revision-counter row per `TenantKind`, holding no tenant data at all. It exists solely so two transactions competing for the same tier's provisioning slot write a *shared* document and therefore conflict — see [Provisioning policy](#provisioning-policy-admin-managed). The unique `kind` index is load-bearing for exactly the same reason as `tenant_defaults`'s. Every read/write carries an inline `//tenantscope:allow`. |
...
**Completion is judged by the CAS, never by having held the lease** — the same rule the saga's own executor follows. `FinishReconcile` returns `ok=false` when the completion CAS does not match, which means this replica no longer held the reconcile lease when it tried to publish the version: its lease expired mid-run and another replica took over, so this one has been performing reconciliation writes *without* the lease, concurrently with the new owner. That boolean **must not be discarded**. Returning success on it would let the replica start serving on the strength of "we held the lease and our writes returned nil", while the version stays unstamped and every later boot re-runs the migration. `reconcileSetupState` instead logs a WARN and loops back to the record: either the new owner publishes the version and the read at the top returns, or its lease lapses and this replica takes over and re-runs — always safe, because every step is idempotent. It does not sleep before looping; the re-entry either returns at once or falls through to the lease claim, whose own `!ok` branch does the waiting. `reconcileLeaseTTL` / `reconcileWaitInterval` are `var`s rather than consts **only** so the lease-expiry tests can shrink them (`reconcile_lease_integration_test.go`); nothing in production assigns to them.
...
1. **`single` is a data invariant in the service, enforced transactionally.** `Service.CreateTenant` (`services/service.go`) counts tenants of the kind occupying a provisioning slot via `repository.CountProvisioningSlotsByKind` and returns the sentinel `services.ErrProvisioningLocked` when one already exists. This covers **all** paths — `POST /v1/tenants`, divisions, and lazy provisioning — automatically. The first tenant on a fresh install counts 0 and passes.

   **The count and the row insert commit as ONE transaction against the `tenant_provisioning_locks` row for the kind** (`repository.RunProvisioningGuarded`), and the reasoning behind that is worth keeping, because two plausible simpler designs are both wrong:

   - *A unique index cannot express it.* `single` is a runtime config an operator toggles, so the schema must keep permitting several tenants per tier. The constraint is a **count**, and a count has to read before it writes.
   - *A transaction alone does not fix the TOCTOU either.* MongoDB transactions are snapshot-isolated with **no read-write conflict detection**, so two concurrent creations each count zero, each insert a *different* tenant document, collide on nothing, and both commit. This is the same shape as the first-assignment hole in [Default tenant invariants](#default-tenant-invariants) — and it is not theoretical: with the plain count-then-insert, 5 of 6 concurrent racers created a Tier-1 tenant in `single` mode.

   What closes it is the shared lock row: both transactions bump it first, MongoDB raises a WriteConflict, and `session.WithTransaction` retries the loser against a snapshot that now includes the winner's committed tenant. **Only the tenant-row write is guarded** — the dependent steps (KMS, membership, owner binding) make external calls and stay outside the transaction, each still unwinding the row on failure. **`manual` mode keeps the plain insert**: it has no cardinality to enforce, and that also keeps ordinary tenant creation working on a standalone mongod, where transactions are unavailable. **The restore branch of `reconcileSetupTenant` takes the identical guard** (a restore re-occupies a slot, so a restore racing a creation must not land the tier at two). Regression tests: `services/provisioning_single_integration_test.go` (create/create, create/restore, and the `manual`-mode boundary).
