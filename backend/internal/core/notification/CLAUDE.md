...
This is pre-existing behavior this branch inherited and deliberately left alone to keep this branch's scope to the one-click work, not a general sentinel-mapping cleanup. A follow-up that wants `Send`/`SendTemplated` to honor these two `iface` sentinels needs to route the dispatch chokepoint's failures through the same mapping `PreflightDelivery` already applies — `mapResolveErr` for the default arm's `Resolve` failure, and an `iface.ErrSenderNotConfigured` literal (or an equivalent mapper) at `dispatchEmail`'s own `usableDriver` failure. Merely reordering or trimming `trustedSentinels` would not do this: the `iface` pair isn't in the list to begin with, so removing the local entries would just leave `newDispatchError` falling back to the generic `ErrSendFailed` sentinel for these cases, not surface the missing `iface` ones.
