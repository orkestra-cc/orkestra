...
**Two known error-mapping gaps this branch did not fix.** `iface.ErrSenderNotConfigured` and `iface.ErrNoSenderForCategory` are unreachable from `Send`/`SendTemplated`: `trustedSentinels` lists the module-local `ErrSenderNotConfigured`/`ErrNoSenderForCategory` *before* their `iface` counterparts, and the default (category-routing) dispatch arm never runs its error through `mapResolveErr` the way `PreflightDelivery`'s default arm does — only the explicit-`Sender` arm maps through `mapBySlugErr`. `newDispatchError`'s scan over `trustedSentinels` therefore always matches the local sentinel first, so a consumer's `errors.Is(err, iface.ErrSenderNotConfigured)` or `errors.Is(err, iface.ErrNoSenderForCategory)` never matches a real `Send`/`SendTemplated` failure — only `PreflightDelivery`, which does map through both functions, honors those two `iface` sentinels today. This is pre-existing behavior this branch inherited and deliberately left alone to keep this branch's scope to the one-click work, not a general sentinel-mapping cleanup; a follow-up should either route the default arm's error through `mapResolveErr` before `failSend`, or drop the two local sentinels from `trustedSentinels` so the `iface` ones win.
