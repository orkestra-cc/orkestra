...
│   │   ├── browserNavigation.ts # the `assign` seam tests spy on — its own import-free module because auth.ts and authedFetch.ts both need it and auth.ts already imports authedFetch (a cycle otherwise); auth.ts re-exports it, same object identity
...
| 1b  | `code: reauthentication_required` — the no-factor branch of the backend's enrolment-proof gate                                                  | clear token **and** marker, then `browserNavigation.assign` to `/login?next=<sanitizeNext(current path)>`; no refresh, no retry |
...
**Branch 1b is the ONE code this wrapper answers with a navigation, and it is a deliberate exception to the rule above it.** The `TERMINAL_CODES` comment forbids `if (body.code) → clear` because four of the middleware's 401 codes refuse an _action_ on a live session, and signing a user out for being asked to confirm a password is absurd. `reauthentication_required` (spec §4.2 D14, `WWW-Authenticate: Bearer error="reauthentication_required"`, body carrying `maxAgeSeconds` + `authTime`) is not one of those: it is not "prove it's you again", it is "**this session is too old to add a second factor**", and the only thing that answers it is a new interactive sign-in — a step-up needs a factor the caller does not have, and a password reconfirm is refused for an MFA-obligated account inside its grace window and impossible for an OAuth-only one. So it does **not** join `TERMINAL_CODES` — that set means "the session is over on the server", and this session is alive — but it does end the session **locally**, for two concrete reasons rather than by analogy: `LoginPage` forwards an already-authenticated visitor straight to its `?next=`, so redirecting without clearing would bounce the user back to the page that just refused them; and the **session marker** is what makes the cold load after the full-page navigation attempt a silent refresh, while a token minted from the same refresh cookie carries `auth_time` **forward unchanged**, so a silent restore would return them to the same refusal with nothing to do about it. The `?next=` is the same convention `RequireAuth` stamps and `LoginPage` consumes, through the same `sanitizeNext` gate. It sits **ahead of the replay guard** because a `reauthentication_required` 401 on a live token would otherwise return there untouched: the guard asks "may this be retried", and retrying does not age a session backwards. Its sibling `step_up_required` is unchanged — inline error copy on the page, no navigation, no clear.

**`MfaEnrolPage` reads `GET /v1/auth/client/me/mfa` before it offers a wizard**, on the shared `['mfa-status']` key so arriving from `AccountSecurityPage` costs no second read. `status === 'enrolled'` renders an "already set up" panel pointing at the operator's admin reset instead of a start button, because an enrolled user is _replacing_ a factor and the backend gate answers a replacement with `step_up_required` — a code this SPA has no modal for and **deliberately is not growing one** (spec §4.2 D14: the client tier is first-enrolment-only until it does). The read **fails open**: only the definite `enrolled` hides the wizard, so a status blip is never what stops a first enrolment — this is UX, not enforcement, and the gate is the backend's either way. Any test that renders this page must stub that endpoint; `onUnhandledRequest: "error"` fails a suite with no failing assertion. The page also **suppresses its own error copy** for a `reauthentication_required` 401 on either enrolment call (`leavingForSignIn`, over `apiErrorCode`): branch 1b has already started a **full-document** navigation, which is slower than the operator console's router transition, so the copy would sit there explaining a page the reader is being taken off. The operator console's two enrolment dialogs make the same call; this is the matching decision, not an oversight.
