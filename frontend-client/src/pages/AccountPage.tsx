  ...
  return (
    <section className="mx-auto max-w-3xl px-6 py-16">
      <header className="mb-10 flex items-start justify-between">
        <div>
          <h1 className="mb-2 text-3xl font-semibold tracking-tight">{t('account.title')}</h1>
      ...
      {me && (
        <dl className="grid grid-cols-1 gap-6 rounded-lg border border-slate-200 bg-white p-6 shadow-sm sm:grid-cols-2">
          <Field label={t('account.fullName')} value={me.fullName ?? '—'} />
          <Field label={t('account.email')} value={me.email} />
          <Field
            label={t('account.emailVerified')}
            value={me.emailVerified ? t('account.verified') : t('account.notVerified')}
          />
